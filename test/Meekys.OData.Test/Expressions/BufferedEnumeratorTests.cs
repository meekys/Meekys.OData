...
{
    public class BufferedEnumeratorTests
    {
        ...
        public void Test_Peek_Normal_And_Past_Buffer()
        {
            ...
            Assert.StartsWith("Offset is larger than buffer size", canPeekEx.Message);
            
            var peekEx = Assert.Throws<ArgumentException>(() => (object)_enumerator.Peek(5));
            Assert.StartsWith("Offset is larger than buffer size", peekEx.Message);
        ...
        public void Test_History_At_End_Of_Iterator()
        {
            ...
            Assert.StartsWith("Offset is larger than buffer size", hasPrevEx.Message);
            var prevEx = Assert.Throws<ArgumentException>(() => (object)_enumerator.Previous(5));
            Assert.StartsWith("Offset is larger than buffer size", prevEx.Message);
            Assert.Equal(_fetchCount, 0);
        }
